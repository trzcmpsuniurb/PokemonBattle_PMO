...
public class FightController {
	...
	public void initialize() throws IOException{
		...
		if(GameControllerStatic.getGameControllerStatic().isMultiPlayer()==false && !GameControllerStatic.getGameControllerStatic().isLeagueison()) {
			...
			fight = new Fight(GameControllerStatic.getGameControllerStatic().getTrainer() , new Bot("Ash",pokemons));
    ...
    void attack(ActionEvent event) throws FileNotFoundException {
		...
		if(GameControllerStatic.getGameControllerStatic().isMultiPlayer()==false && fight.getCurrentTrainer() instanceof Bot) {
			...
			capUsed = fight.getCurrentTrainer().getPokemon().getMoves()[Integer.parseInt(button.getId().subSequence(3, 4).toString()) - 1];
		}	
    ...
    void fight(ActionEvent event) {
    	...
    	HashMap<Button, Move> map = new HashMap<Button, Move>()
    	{{
    		put(cap1Button,fight.getTrainer1().getPokemon().getMoves()[0]);
    		put(cap2Button,fight.getTrainer1().getPokemon().getMoves()[1]);
    		put(cap3Button,fight.getTrainer1().getPokemon().getMoves()[2]);
    		put(cap4Button,fight.getTrainer1().getPokemon().getMoves()[3]);
    	}};
    	
    	//SET DISPLAY
    	for(Map.Entry<Button, Move> entry : map.entrySet()) {
	...
	private void printPokemon(Pokemon poke,int i) throws FileNotFoundException {
		if(i==0) {
	    	...
	    	Move cap1=poke.getMoves()[0];
	    	Move cap2=poke.getMoves()[1];
	    	Move cap3=poke.getMoves()[2];
	    	Move cap4=poke.getMoves()[3];
